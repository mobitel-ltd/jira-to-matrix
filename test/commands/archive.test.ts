...
const { getAliases } from '../../src/bot/settings');
...
const { deleteAlias, KICK_ALL_OPTION, PERSONAL_REPO_OPTION } from '../../src/bot/commands/command-list/archive');
const {
    DEFAULT_REMOTE_NAME,
    DEFAULT_EXT,
    EVENTS_DIR_NAME,
...
describe('Archive command', () => {
    ...
    describe('archive with export', () => {
        ...
        beforeEach(async () => {
            const _repoName = chatSingle.getChatUserId(adminSender.name).replace(/[^a-z0-9_.-]+/g, '__');
        ...
        it('expect command succeded', async () => {
            ...
            expect(chatSingle.sendHtmlMessage).to.be.calledWithExactly(roomId, expectedMsg, expectedMsg);
        });
        ...
        it('expect command succeded with custom repo name as option and kick all option', async () => {
            ...
            expect(chatSingle.deleteRoomAlias).not.to.be.called;
            expect(chatSingle.leaveRoom).to.be.calledWithExactly(roomData.id);
            const expectedMsg = [
        ...
        it('expect command succeded and all users are kicked if not other admins but alias is not deleted but saved', async () => {
            ...
            expect(chatSingle.deleteRoomAlias).to.be.calledWithExactly(roomData.alias);
        ...
        it('expect command succeded but bot cannot kick anybody if power is less than 100', async () => {
            ...
            expect(mediaFiles).to.have.length(expectedMediaFileNames.length);
            expect(mediaFiles).to.have.deep.members(expectedMediaFileNames);
            expect(chatSingle.kickUserByRoom).not.to.be.called;
            expect(chatSingle.deleteRoomAlias).not.to.be.called;
            expect(chatSingle.sendHtmlMessage).to.be.calledWithExactly(roomId, expectedMsg, expectedMsg);
        });

        it('command cannot be succeded if such project is not exists in git repo', async () => {
            chatSingle.getDownloadLink.throws();
            const result = await commandHandler({
                ...baseOptions,
                config: configWithTmpPath,
                sender: adminSender.name,
