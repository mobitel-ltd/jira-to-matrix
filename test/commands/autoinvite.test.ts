...
describe('Invite setting for projects', () => {
    ...
    beforeEach(() => {
        ...
        chatApi = testUtils.getChatClass({ existedUsers: [correctUser, anotherCorrectUser] });
    ...
    it('Success add key', async () => {
        const post = translate('autoinviteKeyAdded', {
            projectKey,
            matrixUserFromCommand: await chatSingle.getChatUserId(correctUser),
    ...
    it('Success add key admin (not lead)', async () => {
        const post = translate('autoinviteKeyAdded', {
            projectKey,
            matrixUserFromCommand: chatSingle.getChatUserId(correctUser),
            typeTaskFromUser: issueType,
        });
