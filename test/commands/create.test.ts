...
const postNewLinksbody from '../fixtures/webhooks/issuelink/created_relates.json');
const issueLinkBody from '../fixtures/jira-api-requests/issuelinkRelates.json');
const issueBody from '../fixtures/jira-api-requests/issue.json');

const postNewLinks from '../../src/bot/actions/post-new-links';
const { getPostNewLinksData } from '../../src/jira-hook-parser/parse-body';
const { getPostLinkMessageBody } from '../../src/bot/actions/helper');
...
describe('create test', () => {
    ...
    beforeEach(() => {
        ...
        chatSingle.getRoomId.withArgs(utils.getInwardLinkKey(issueLinkBody)).resolves(roomId);
        chatSingle.getRoomId.withArgs(utils.getOutwardLinkKey(issueLinkBody)).resolves(roomId);
    ...
    it('Expect message with list task types for current project IF command !create called without any params', async () => {
        ...
        expect(chatSingle.sendHtmlMessage).to.be.called;
    ...
    it('Expect create new issue and receive hook "new link - relates to" IF command "!create TestTypeTask" with correct type issue and correct new issue name', async () => {
        ...
        expect(chatSingle.sendHtmlMessage).to.be.calledWithExactly(roomId, body.body, body.htmlBody);
    });
