...
export const projectarchive = async ({ bodyText, chatApi, roomData, taskTracker }) => {
    if (!chatApi.isMaster()) {
    ...
    if (!(await taskTracker.isJiraPartExists(projectKey))) {
    ...
    if (textOptions.has(STATUS_OPTION)) {
        ...
        if (!(await taskTracker.hasStatusInProject(projectKey, status))) {
