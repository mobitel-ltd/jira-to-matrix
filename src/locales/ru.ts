...
export const dict = Object.freeze({
    ...
    issueAddedToMilestone: '%{user} добавил добавил задачу [%{summary}](%{viewUrl}) к майлстоуну',
    newLink: 'Новый линк',
    ...
    noOptions: 'Команда не содежит ни одной опции',
    noSelfKick: 'Бот не будет выкидывать из комнаты самого себя',
    userKicked: 'Пользователь "%{userId}" был исключен из комнаты',
    kickFail: 'Произошла ошибка, пользователь "%{userId}" не был исключен',
    oneOptionOnly: 'Команда может быть использована только с одной опцией',
    allKicked: 'Пользователи, не являющиеся администраторами успешно исключены из комнаты',
    forbiddenAdminKick: 'Пользователь "%{userId}" является администратором данной комнаты и не может быть исключен',
});
